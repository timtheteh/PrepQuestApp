...
export const generateFileUploadPrompt = (params: FileUploadPromptParams): string => {
  ...
  if (pdfCaptionClaudeCaption && language === 'Hungarian') {
    prompt += `Itt vannak további információk és kontextus egy PDF fájlból a felkészülésemhez: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Hungarian') {
    prompt += `Itt vannak további információk és kontextus egy szöveges fájlból a felkészülésemhez: ${extractedText}\n`;
  }
  if (imageCaptionClaudeCaption && language === 'Hungarian') {
    prompt += `Itt vannak további információk és kontextus néhány képről a felkészülésemhez: ${imageCaptionClaudeCaption}\n`;
  }
  if (pdfCaptionClaudeCaption && language === 'Norwegian') {
    prompt += `Her er tilleggsinformasjon og kontekst fra en PDF-fil for min forberedelse: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Norwegian') {
    prompt += `Her er tilleggsinformasjon og kontekst fra en tekstfil for min forberedelse: ${extractedText}\n`;
  }
  if (imageCaptionClaudeCaption && language === 'Norwegian') {
    prompt += `Her er tilleggsinformasjon og kontekst fra noen bilder for min forberedelse: ${imageCaptionClaudeCaption}\n`;
  }
  if (pdfCaptionClaudeCaption && language === 'Polish') {
    prompt += `Oto dodatkowe informacje i kontekst z pliku PDF dla mojego przygotowania: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Polish') {
    prompt += `Oto dodatkowe informacje i kontekst z pliku tekstowego dla mojego przygotowania: ${extractedText}\n`;
  }
  if (imageCaptionClaudeCaption && language === 'Polish') {
    prompt += `Oto dodatkowe informacje i kontekst z niektórych obrazów dla mojego przygotowania: ${imageCaptionClaudeCaption}\n`;
  }
  if (pdfCaptionClaudeCaption && language === 'Portuguese') {
    prompt += `Aqui estão informações adicionais e contexto de um arquivo PDF para minha preparação: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Portuguese') {
    prompt += `Aqui estão informações adicionais e contexto de um arquivo de texto para minha preparação: ${extractedText}\n`;
  }
  if (imageCaptionClaudeCaption && language === 'Portuguese') {
    prompt += `Aqui estão informações adicionais e contexto de algumas imagens para minha preparação: ${imageCaptionClaudeCaption}\n`;
  }
  if (pdfCaptionClaudeCaption && language === 'Romanian') {
    prompt += `Iată informații și context suplimentare dintr-un fișier PDF pentru pregătirea mea: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Romanian') {
    prompt += `Iată informații și context suplimentare dintr-un fișier text pentru pregătirea mea: ${extractedText}\n`;
  }
  if (imageCaptionClaudeCaption && language === 'Romanian') {
    prompt += `Iată informații și context suplimentare din unele imagini pentru pregătirea mea: ${imageCaptionClaudeCaption}\n`;
  }
  if (pdfCaptionClaudeCaption && language === 'Finnish') {
    prompt += `Tässä on lisätietoja ja kontekstia PDF-tiedostosta valmistautumistani varten: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Finnish') {
    prompt += `Tässä on lisätietoja ja kontekstia tekstitiedostosta valmistautumistani varten: ${extractedText}\n`;
  }
  ...
  if (pdfCaptionClaudeCaption && language === 'Tagalog') {
    prompt += `Narito ang karagdagang impormasyon at konteksto mula sa isang PDF file para sa aking paghahanda: ${pdfCaptionClaudeCaption}\n`;
  }
  if (extractedText && extractedText.trim() !== '' && language === 'Tagalog') {
    prompt += `Narito ang karagdagang impormasyon at konteksto mula sa isang text file para sa aking paghahanda: ${extractedText}\n`;
  }
  if (imageCaptionClaudeCaption && language === 'Tagalog') {
    prompt += `Narito ang karagdagang impormasyon at konteksto mula sa ilang mga larawan para sa aking paghahanda: ${imageCaptionClaudeCaption}\n`;
  }
  ...
  if (distribution) {
    if (language === 'English') {
      ...
      for (const [flashcardType, numQuestions] of Object.entries(distribution)) {
        ...
        // @ts-ignore
        prompt += `${promptAndDataNorwegian[flashcardType].prompt}\n`;
      }
    } else if (language === 'Polish') {
      for (const [flashcardType, numQuestions] of Object.entries(distribution)) {
        prompt += `Wygeneruj ${numQuestions} fiszek typu '${flashcardType}'.\n`;
        // @ts-ignore
        prompt += `${promptAndDataPolish[flashcardType].prompt}\n`;
      }
    } else if (language === 'Portuguese') {
      for (const [flashcardType, numQuestions] of Object.entries(distribution)) {
        prompt += `Gere ${numQuestions} cartões do tipo '${flashcardType}'.\n`;
        // @ts-ignore
        prompt += `${promptAndDataPortuguese[flashcardType].prompt}\n`;
      }
    } else if (language === 'Romanian') {
      for (const [flashcardType, numQuestions] of Object.entries(distribution)) {
        prompt += `Generează ${numQuestions} carduri de tipul '${flashcardType}'.\n`;
        // @ts-ignore
        prompt += `${promptAndDataRomanian[flashcardType].prompt}\n`;
      }
  ...
  if (language === 'Hungarian' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Győződjön meg róla, hogy értelmes, átgondolt és valószínű kérdéseket és válaszokat generál, amelyek specifikusak az interjúmra és a munkakörömre.\n';
    prompt += 'Azonban RENDKÍVÜL FONTOS, HOGY NE TÉRJEN EL az információktól és kontextustól, amelyet a PDF fájlból, szöveges fájlból vagy képekből biztosítottam. MARADJON CSAK A PDF FÁJL, SZÖVEGES FÁJL VAGY KÉPEK TARTALMÁNÁL. ';
    prompt += 'Generáljon egy JSON tömböt kártyákból ebben a formátumban: [{"flashcardType": <>, "question": <>, "answer": <>}], ahol minden {"flashcardType": <>, "question": <>, "answer": <>} egy kártyát képvisel.';
  }
  ...
  if (language === 'Norwegian' && modeStr === 'interview' && isAIGenerate) {
  ...
  if (language === 'Norwegian' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Sørg for å generere meningsfulle, gjennomtenkte og sannsynlige spørsmål og svar spesifikke for mitt intervju og min jobbrolle.\n';
    prompt += 'Imidlertid er det EKSTREMT VIKTIG AT DU IKKE AVIKER fra informasjonen og konteksten jeg har gitt fra PDF-filen, tekstfilen eller bildene. HOLD DEG KUN TIL INNHOLDET FRA PDF-FILEN, TEKSTFILEN ELLER BILDENE. ';
    prompt += 'Generer en JSON-array av kort i dette formatet: [{"flashcardType": <>, "question": <>, "answer": <>}], hvor hver {"flashcardType": <>, "question": <>, "answer": <>} representerer et kort.';
  }
  if (language === 'Norwegian' && modeStr === 'study' && isAIGenerate) {
  ...
  if (language === 'Polish' && modeStr === 'interview' && isAIGenerate) {
  ...
  if (language === 'Polish' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Upewnij się, że generujesz znaczące, przemyślane i prawdopodobne pytania i odpowiedzi specyficzne dla mojej rozmowy i mojej roli zawodowej.\n';
    prompt += 'Jednak NIEZWYKLE WAŻNE JEST, ABYŚ NIE ODCHODZIŁ od informacji i kontekstu, które podałem z pliku PDF, pliku tekstowego lub obrazów. TRZYMAJ SIĘ TYLKO TREŚCI Z PLIKU PDF, PLIKU TEKSTOWEGO LUB OBRAZÓW. ';
    prompt += 'Wygeneruj tablicę JSON fiszek w tym formacie: [{"flashcardType": <>, "question": <>, "answer": <>}], gdzie każda {"flashcardType": <>, "question": <>, "answer": <>} reprezentuje fiszkę.';
  }
  if (language === 'Polish' && modeStr === 'study' && isAIGenerate) {
  ...
  if (language === 'Portuguese' && modeStr === 'interview' && isAIGenerate) {
  ...
  if (language === 'Portuguese' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Certifique-se de gerar perguntas e respostas significativas, reflexivas e prováveis específicas para minha entrevista e para minha função de trabalho.\n';
    prompt += 'No entanto, é EXTREMAMENTE CRUCIAL QUE VOCÊ NÃO SE DESVIE das informações e do contexto que forneci do arquivo PDF, arquivo de texto ou imagens. MANTENHA-SE APENAS NO CONTEÚDO DO ARQUIVO PDF, ARQUIVO DE TEXTO OU IMAGENS. ';
    prompt += 'Gere um array JSON de cartões neste formato: [{"flashcardType": <>, "question": <>, "answer": <>}], onde cada {"flashcardType": <>, "question": <>, "answer": <>} representa um cartão.';
  }
  if (language === 'Portuguese' && modeStr === 'study' && isAIGenerate) {
  ...
  if (language === 'Romanian' && modeStr === 'interview' && isAIGenerate) {
  ...
  if (language === 'Romanian' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Asigură-te că generezi întrebări și răspunsuri semnificative, gândite și probabile specifice pentru interviul meu și pentru rolul meu de muncă.\n';
    prompt += 'Cu toate acestea, este EXTREM DE CRUCIAL SĂ NU DEVIEZI de la informațiile și contextul pe care le-am furnizat din fișierul PDF, fișierul text sau imaginile. RĂMÂI DOAR LA CONȚINUTUL DIN FIȘIERUL PDF, FIȘIERUL TEXT SAU IMAGINILE. ';
    prompt += 'Generează un array JSON de carduri în acest format: [{"flashcardType": <>, "question": <>, "answer": <>}], unde fiecare {"flashcardType": <>, "question": <>, "answer": <>} reprezintă un card.';
  }
  if (language === 'Romanian' && modeStr === 'study' && isAIGenerate) {
  ...
  if (language === 'Finnish' && modeStr === 'interview' && isAIGenerate) {
  ...
  if (language === 'Finnish' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Varmista, että luot merkityksellisiä, harkittuja ja todennäköisiä kysymyksiä ja vastauksia, jotka ovat erityisiä haastattelulleni ja työroolilleni.\n';
    prompt += 'On kuitenkin ERIITTÄIN TÄRKEÄÄ, ETTÄ ET POIKKEA tiedoista ja kontekstista, jonka olen antanut PDF-tiedostosta, tekstitiedostosta tai kuvista. PIDÄ KIINNI VAIN PDF-TIEDOSTON, TEKSTITIEDOSTON TAI KUVIEN SISÄLLÖSTÄ. ';
    prompt += 'Luo JSON-taulukko kortteja tässä muodossa: [{"flashcardType": <>, "question": <>, "answer": <>}], jossa jokainen {"flashcardType": <>, "question": <>, "answer": <>} edustaa korttia.';
  }
  if (language === 'Finnish' && modeStr === 'study' && isAIGenerate) {
  ...
  if (language === 'Tagalog' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Siguraduhing lumikha ng makabuluhan, maingat at malamang na mga tanong at sagot na partikular para sa aking interview at sa aking trabaho.\n';
    prompt += 'Gayunpaman, LUBHANG MAHALAGA NA HINDI KA LUMIHIS mula sa impormasyon at konteksto na ibinigay ko mula sa PDF file, text file o mga larawan. MANATILI LAMANG SA NILALAMAN NG PDF FILE, TEXT FILE O MGA LARAWAN. ';
    prompt += 'Lumikha ng JSON array ng flashcards sa format na ito: [{"flashcardType": <>, "question": <>, "answer": <>}], kung saan ang bawat {"flashcardType": <>, "question": <>, "answer": <>} ay kumakatawan sa isang flashcard.';
  }
