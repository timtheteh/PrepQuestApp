...
export const generateYouTubeLinkPrompt = (params: YouTubeLinkPromptParams): string => {
  ...
  if (language === 'English') {
    ...
    prompt += `Her er tilleggsinformasjon og kontekst fra en YouTube-videotranskript for min forberedelse: ${transcript}\n`;
  } else if (language === 'Polish') {
    prompt += `Oto dodatkowe informacje i kontekst z transkrypcji wideo YouTube dla mojego przygotowania: ${transcript}\n`;
  } else if (language === 'Portuguese') {
    prompt += `Aqui estão informações adicionais e contexto de uma transcrição de vídeo do YouTube para minha preparação: ${transcript}\n`;
  ...
  if (language === 'Italian' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Assicurati di generare domande e risposte significative, riflessive e probabili specifiche per il mio colloquio e per il mio ruolo lavorativo.\n';
    prompt += 'Tuttavia, è ESTREMAMENTE CRUCIALE CHE NON TI DISCOSTI dalle informazioni e dal contesto che ho fornito dalla trascrizione video YouTube. RESTA SOLO SUL CONTENUTO DELLA TRASCRIZIONE. ';
    prompt += 'Genera un array JSON di carte in questo formato: [{"flashcardType": <>, "question": <>, "answer": <>}], dove ogni {"flashcardType": <>, "question": <>, "answer": <>} rappresenta una carta.';
  }
  ...
  if (language === 'Norwegian' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Sørg for å generere meningsfulle, gjennomtenkte og sannsynlige spørsmål og svar spesifikke for mitt intervju og min jobbrolle.\n';
    prompt += 'Imidlertid er det EKSTREMT VIKTIG AT DU IKKE AVIKER fra informasjonen og konteksten jeg har gitt fra YouTube-videotranskriptet. HOLD DEG KUN TIL INNHOLDET FRA TRANSKRIPTET. ';
    prompt += 'Generer en JSON-array av kort i dette formatet: [{"flashcardType": <>, "question": <>, "answer": <>}], hvor hver {"flashcardType": <>, "question": <>, "answer": <>} representerer et kort.';
  }
  ...
  if (language === 'Polish' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Upewnij się, że generujesz znaczące, przemyślane i prawdopodobne pytania i odpowiedzi specyficzne dla mojej rozmowy i mojej roli zawodowej.\n';
    prompt += 'Jednak NIEZWYKLE WAŻNE JEST, ABYŚ NIE ODCHODZIŁ od informacji i kontekstu, które podałem z transkrypcji wideo YouTube. TRZYMAJ SIĘ TYLKO TREŚCI Z TRANSKRYPCJI. ';
    prompt += 'Wygeneruj tablicę JSON fiszek w tym formacie: [{"flashcardType": <>, "question": <>, "answer": <>}], gdzie każda {"flashcardType": <>, "question": <>, "answer": <>} reprezentuje fiszkę.';
  }
  ...
  if (language === 'Portuguese' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Certifique-se de gerar perguntas e respostas significativas, reflexivas e prováveis específicas para minha entrevista e para minha função de trabalho.\n';
    prompt += 'No entanto, é EXTREMAMENTE CRUCIAL QUE VOCÊ NÃO SE DESVIE das informações e do contexto que forneci da transcrição do vídeo do YouTube. MANTENHA-SE APENAS NO CONTEÚDO DA TRANSCRIÇÃO. ';
    prompt += 'Gere um array JSON de cartões neste formato: [{"flashcardType": <>, "question": <>, "answer": <>}], onde cada {"flashcardType": <>, "question": <>, "answer": <>} representa um cartão.';
  }
  ...
  if (language === 'Romanian' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Asigură-te că generezi întrebări și răspunsuri semnificative, gândite și probabile specifice pentru interviul meu și pentru rolul meu de muncă.\n';
    prompt += 'Cu toate acestea, este EXTREM DE CRUCIAL SĂ NU DEVIEZI de la informațiile și contextul pe care le-am furnizat din transcrierea video YouTube. RĂMÂI DOAR LA CONȚINUTUL DIN TRANSCRIERE. ';
    prompt += 'Generează un array JSON de carduri în acest format: [{"flashcardType": <>, "question": <>, "answer": <>}], unde fiecare {"flashcardType": <>, "question": <>, "answer": <>} reprezintă un card.';
  }
  ...
  if (language === 'Finnish' && modeStr === 'interview' && !isAIGenerate) {
    prompt += 'Varmista, että luot merkityksellisiä, harkittuja ja todennäköisiä kysymyksiä ja vastauksia, jotka ovat erityisiä haastattelulleni ja työroolilleni.\n';
    prompt += 'On kuitenkin ERIITTÄIN TÄRKEÄÄ, ETTÄ ET POIKKEA tiedoista ja kontekstista, jonka olen antanut YouTube-videon tekstityksestä. PIDÄ KIINNI VAIN TEKSTITYKSEN SISÄLLÖSTÄ. ';
    prompt += 'Luo JSON-taulukko kortteja tässä muodossa: [{"flashcardType": <>, "question": <>, "answer": <>}], jossa jokainen {"flashcardType": <>, "question": <>, "answer": <>} edustaa korttia.';
  }
